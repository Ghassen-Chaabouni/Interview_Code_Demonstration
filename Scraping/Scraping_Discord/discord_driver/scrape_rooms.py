...
class ScrapeRooms:
    def __init__(self, driver, context, json_parser=False, game = "", start_date = "", end_date = "", key = ""):
    ...
    def scrape(self):
        ...
        while(1):
            for i in range (len(group)):
                ...
                room_name = page.find_all("h3", class_="name-2S4ebg")[i]    # Get the room name.
                ...
                scrape_chosen_message = ScrapeChosenMessages.ScrapeChosenMessages(self.driver, self.context, self.json_parser, self.start_date, self.end_date, room_name, self.key)   # Search for words and scrape conversations

                if(self.json_parser):
                    scrape_chosen_message.scrape()
                else:
                    data_json.append(scrape_chosen_message.scrape())
            ...
            # try:
                # next_page = self.driver.find_elements_by_class_name('endButtonInner-36a1wi')[1]       # Find the next page button.
