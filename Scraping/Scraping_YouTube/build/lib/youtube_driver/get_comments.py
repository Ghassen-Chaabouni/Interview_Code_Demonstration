        ...
        while(self.response):
            ...
            for (index,item) in enumerate(self.response['items']):
                ...
                if(self.take_comments):
                    number_of_replies = str(item['snippet']['totalReplyCount']).encode('utf-8')    # Get the number of replies.

                    is_public = str(item['snippet']['isPublic']).encode('utf-8')   # Get whether the thread, including all of its comments and comment replies, is visible to all YouTube users.
                    can_reply = str(item['snippet']['canReply']).encode('utf-8')   # Get whether the current viewer can reply to the thread.
                    author = str(item['snippet']['topLevelComment']['snippet']['authorDisplayName']).encode('utf-8')   # Get the name of the person who wrote the message.
                    display_text = str(item['snippet']['topLevelComment']['snippet']['textDisplay']).encode('utf-8')   # Get the message in the html format.
                    try:
                        author_profile_image = str(item['snippet']['topLevelComment']['snippet']['authorProfileImageUrl']).encode('utf-8')                 
                    except:
                        author_profile_image = ""
                    original_text = str(item['snippet']['topLevelComment']['snippet']['textOriginal']).encode('utf-8') # Get The original, raw text of the comment as it was initially posted or last updated.
                    likes = str(item['snippet']['topLevelComment']['snippet']['likeCount']).encode('utf-8')   # Get the total number of likes (positive ratings) the comment has received.
                    published_time = str(item['snippet']['topLevelComment']['snippet']['publishedAt']).encode('utf-8')   # Get the date and time when the comment was orignally published. The value is specified in ISO 8601 format.
                    update_time = str(item['snippet']['topLevelComment']['snippet']['updatedAt']).encode('utf-8')        # Get the date and time when the comment was last updated. The value is specified in ISO 8601 format.
                    viewer_rating = str(item['snippet']['topLevelComment']['snippet']['viewerRating']).encode('utf-8')   # Get the rating value: "like" if the viewer has rated the comment positively, "none" in all other cases.
                    try:
                        replies = str(item['replies']).encode('utf-8')
                    except:
                        replies = ""
                ...
                if(self.take_comments):
                    while(response2):
                        ...
                        for (index2,item2) in enumerate(response2['items']):
                            ...
                            publishedAt = str(item2['snippet']['publishedAt']).encode('utf-8')  # Get the date and time that the video was published.
                            ...
                            title = str(item2['snippet']['title']).encode('utf-8')  # Get the video's title.
                            video_like = str(item2['statistics']['likeCount']).encode('utf-8')

                            try:
                                description = str(item2['snippet']['description']).encode('utf-8')  # Get the video's description.
                            except:
                                description = "No description"

                            thumbnails = str(item2['snippet']['thumbnails']["default"]["url"]).encode('utf-8')   # Get the video's thumbnail.
                            channelTitle = str(item2['snippet']['channelTitle']).encode('utf-8')   # Get the channel name which the video belongs to.

                            try:
                                tags = str(item2['snippet']['tags']).encode('utf-8')    # Get the video's tags.
                            except:
                                tags = "No tags"

                            contentDetails = str(item2['contentDetails']['duration']).encode('utf-8')   # Get the length of the video. The property value is an ISO 8601 duration.
